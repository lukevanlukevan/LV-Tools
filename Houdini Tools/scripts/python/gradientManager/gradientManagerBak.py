...
class gradientManager(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.graphGrid.setSelectionMode(QtWidgets.QAbstractItemView.ExtendedSelection)
        ...
        for filename in os.listdir(self.gradFolder):
            ...
            if os.path.isfile(f):
                ...
                if json_object['isColor'] == False:
                    self.graph = GraphView(json_object['plot_values'])
                ...
                # self.menu.addAction("Remove", self.removeRamp)

                self.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
                self.customContextMenuRequested.connect(self.menuExec)
    ...
    def removeRamp(self,  *args):
        print(args)
        print("Removing")

    def undoRamp(self):
        count = self.graphGrid.count()
        child = self.graphGrid.itemAt(count-1).widget()
